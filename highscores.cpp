...
void highscores_Show() {
   ...
   clear(screen);
    ...
    for(int i = 0; i < NUMBER_OF_SCORES; i++) {
       textout(screen, comic, highscores.names[i], 50, 100 + (text_height(comic)) * i, -1);
      itoa( highscores.scores[i], scoreStr, 10);
      textout(screen, comic, scoreStr, 500, 100 + (text_height(comic)) * i, -1);
      }
   while(!key[KEY_ENTER]) {
   };
   ...
   if (highscores.scores[0] < score)
    ...
    if (score_rank < NUMBER_OF_SCORES) {
      ...
      // ****** Clear the screen and get the new High Score ******
       ...
       textout_centre(screen, comic, "You got a high score!", SCREEN_W / 2, 50, 2);
      textout_centre(screen, comic, "Please enter your name:", SCREEN_W / 2, 200, 2);
      ...
      while(1) {
         rectfill(screen, SCREEN_W/2 - 54, 246, SCREEN_W/2 + 54, 254 + text_height(comic), 4);
          rectfill(screen, SCREEN_W/2 - 50, 250, SCREEN_W/2 + 50, 250 + text_height(comic), 1);
         textout_centre(screen, comic, highscores.names[score_rank], SCREEN_W / 2, 251, 2);
         keyp = readkey();
